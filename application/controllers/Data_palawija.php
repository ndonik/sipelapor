...
class Data_palawija extends CI_Controller
{
	public function index()
	{
		...
		$this->template->load('index', 'data_palawija/index', $data);
	}
	...
	public function add()
	{
		...
		if ($result) {
			...
			redirect('data_palawija/filter/' . encrypt_url($id_bulan) . '/' . encrypt_url($id_provinsi) . '/' . encrypt_url($tahun));
		} else {
	...
	public function edit($id, $id_bulan, $id_provinsi, $tahun)
	{
		...
		if ($data['data_palawija']) {
			$this->template->load('index', 'data_palawija/edit', $data);
		} else {
			$this->session->set_flashdata('alert', 'Gagal');
			redirect('data_palawija/filter/' . encrypt_url($id_bulan) . '/' . encrypt_url($id_provinsi) . '/' . encrypt_url($tahun));
		}
